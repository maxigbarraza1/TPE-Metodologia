...
public class AccesoBaseDatos {
	...
	public boolean loginMunicipio(String nick, String contraseña) {
		if (this.usuarios.get(nick).getContraseña().equals(contraseña)) {
			// Esto no se haria, no se guarda en Acceso Base Datos, nos quedamos con el nick
			// en la funcionalidadempmunicipio
			// sie es que se devuelve true
			// this.user = new EmpMunicipio(nick, contraseña,
			// usuarios.get(nick).getEmail());
			return true;
		}
		return false;
	}
	...
	public void registrarProducto(String nick, int id, int cantidad, Direccion direccion) {
		...
		// Hago el casting -> busco que la ID sea correcta y que este en los productos
		// reciclables
		// -> busco que el PLI exista -> agrego el producto al PLI y al vector de
		// productos del vecino
	...
	// Busca dentro del mapa de usuarios si el nick ya existe, para que al momento
	// de registarse no
